...
const AllVehicleLocation = () => {
    ...
    const [zoomDestination, setZoomDestination] = useState({
        destinationMap: {},
        destinationList: {}
    })
    ...
    const checkOverlap = useCallback((map) => {
        let marked = {}
        const destiMap = {}
        const destiList = {}
        if (!map) {
        ...
        markers.forEach((marker1, index) => {
            if (marked[marker1.position.toString()]) return;
            marked[marker1.position.toString()] = 1;
            destiMap[marker1.destination] = marker1.destination
            destiList[marker1.destination] = [cargoList[index]]
            markers.forEach((marker2, innerIndex) => {
                if (index < innerIndex) {
                    const dx = Math.abs(marker1.screenCoords.x - marker2.screenCoords.x);
                    const dy = Math.abs(marker1.screenCoords.y - marker2.screenCoords.y);

                    if (dx < tolerance && dy < tolerance) {
                        if (!overlaps[marker1.position.toString()]) {
                            overlaps[marker1.position.toString()] = [];
                        }
                        overlaps[marker1.position.toString()].push(marker2.position.toString());
                        marked[marker2.position.toString()] = 1;
                        destiMap[marker2.destination] = marker1.destination
                        destiList[marker1.destination].push(cargoList[innerIndex])
                    }
                }
            });
        });

        if (zoomLevel != map.getLevel()) {
            //console.log('koybar')
            setDuplicateDestination(undefined)
            setZoomDestination({
                destinationMap: destiMap,
                destinationList: destiList
            })
        }

        return destiList;
    }, [zoomLevel, cargoList, isFetching]);
    ...
    useEffect(() => {
        ...
        if (!isFetching && cargoAllLocation) {
            ...
            const newCargo = [...cargoAllLocation].map((cargo) => {
                ...
                const {
                    current_point: currentPosition,
                    end_point: destination,
                    starting_point: origin,
                    cargo_to_advertisment,
                    logs,
                    start_time,
                    end_time,
                    total_time_covered,
                ...
                return {
                    ...
                    info: {
                        ...
                        total_time_covered,
                        total_distance_covered,
                        truck_number: cargo_to_advertisment?.vehicle_information?.car_number,
    ...
    const renderCargo = (
        ...
        const { destinationList, destinationMap } = zoomDestination;
                ...
                {destination && <MapMarker
                    ...
                    <div style={{ zIndex: 100 }} className="all-location-truck-tooltip-container">
                            ...
                            <div onClick={() => {
                                ...
                                <Paper elevation={24}>
                                    <div style={{ border: '1px solid #FFFFFF' }}
                                         className={'!rounded !shadow-xxl bg-[#FFFFFF] flex flex-col gap-2 w-[260px] p-4'}>
                                        {
                                            isMultiple ?
                                                <div
                                                    className={'text-base text-[#10121d]'}>{allLocation?.duplicateDestination}</div> :
                                                <>
                                                    {
                                                        [{
                                                            title: viewAllLocation?.total_driving_distance,
                                                            value: formatNumberWithCommas(Number(total_distance_covered), 2),
                                                            extra: <span>km</span>
                                                        },
                                                            {
                                                                title: viewAllLocation?.total_driving_time,
                                                                value: total_time_covered
                                                            },
                                                            {
                                                                title: viewAllLocation?.operation_start_time,
                                                                value: `${startTime?.[0]}:${startTime?.[1]}`
                                                            },
                                                            {
                                                                title: viewAllLocation?.operation_end_time,
                                                                value: `${endTime?.[0]}:${endTime?.[1]}`
                                                            }
                                                        ].map((item, index) => <div style={{ zIndex: 100 }} key={index}
                                                                                    className={'flex justify-between items-center gap-2'}>
                                                            <span
                                                                className={`text-[16px] text-[#2C324C] flex-1`}>{item.title}</span>
                                                            <div
                                                                className={'text-[16px] flex-1 flex justify-end gap-1 break-all flex-wrap'}>
                                                                <span
                                                                    className={'font-bold text-advertiser-primary'}>{item.value}</span> {item?.extra && item?.extra}
                                                            </div>
                                                        </div>)
                                                    }
                                                </>
                        ...
                        >
                            ...
                            {
                                isMultiple ? <div
                                        className={'!shrink-0 flex-1 text-sm font-normal text-advertiser-primary flex w-[100%] whitespace-nowrap'}>{destinationList[destinationMap[info.destination]]?.length}</div>
                                    :
                                    <div
                                        className={'flex-1 text-sm font-normal text-advertiser-primary flex w-[100%] whitespace-nowrap'}>{truck_number}</div>
                            }</div>
                    </div>
                    ...
                    cargoList?.length || 0 > 0 ?
                        <Map
                            ...
                            onZoomChanged={(map) => {
                                ...
                                const overlaps = checkOverlap(map); // Check overlaps based on the current zoom level
                                // console.log('Overlaps:', overlaps,map.getLevel());
                            }}
                            onCreate={(map) => {
                                setZoomLevel(map?.getLevel() || 5);
                                const overlaps = checkOverlap(map);
                        ...
                        >
                            {cargoList.map(({
                                ...
                                destination: toLatLng(zoomDestination.destinationMap?.[destination] || destination),
                                currentPosition: toLatLng(currentPosition),
                                name,
                                logs, id: index, info
                            }))}
                        </Map> : !isLoading && <div
                        className={`flex flex-col justify-center items-center align-middle h-[calc(100vh-170px)] ${showDrawer ? 'w-full' : 'w-[calc(100%-340px)]'}`}>
                        <Image
