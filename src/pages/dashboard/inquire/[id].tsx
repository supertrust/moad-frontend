...
export default function InquireDetailScreen() {
  ...
  const [deleteModal, setDeleteModal] = useState<{ open: boolean, text: string | null }>({
    open: false,
    text: null,
  });
  ...
  return (
    <>
      <Head>
        <title>{isLoading ? "Loading page ..." : data !== null ? `[${data?.inquiry_type}] ${data?.inquiry_title}` : 'Data Not Found'}</title>
      </Head>
            ...
            <>
              <div className="px-7 py-3 border-b flex gap-2 justify-between items-center">
                ...
                <div className="flex gap-3 justify-end text-sm">
                  <span>{dateFormat(data?.created_at as string, 'Y-m-d H:i')}</span>
                </div>
              </div>
              <div className="px-7 py-3 max-h-96 overflow-y-auto flex flex-col gap-3">
                ...
                <div className="mt-3 flex justify-end gap-2 !text-sm">
                  {data?.inquiry_answer !== null ?
                    ...
                    : <>
                      <Link href={`/dashboard/inquire/form/${id}`}><span className="text-gray-400">수정</span></Link>
                  ...
                  {data?.inquiry_answer !== null ?
                    <>
                      <div className="flex justify-between gap-3 text-sm">
                        ...
                        <div>{dateFormat(data?.updated_at as string, 'Y-m-d H:i')}</div>
                      </div>
