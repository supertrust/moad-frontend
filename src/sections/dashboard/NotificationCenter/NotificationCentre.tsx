...
export default function NotificationCentre() {
  ...
  const { data: response, isLoading } = useGetAdvertiserVehiclesStats({
    notfication: true,
    ...
    <>
      <div className={`${styles.titleWrap} pb-[12px] lg:pb-[20px] gap-[8px]`}>
        <div className={`${styles.title} p-0`}>
          <span>{dictionary.notificationCenter.title}</span>
        </div>
      ...
      <div
        ...
        <Link
          href='dashboard/notification'
          className={`${
            notifications?.length ? '' : 'pointer-events-none	text-[#999]'
          }`}>
            ...
            {isLoading ? (
              [1, 2, 3, 4].map(
                (each, index) =>
                  index < 4 && (
                    <li
                      className={clsx(styles.list, index == 3 && '!border-b-0')}
                      key={index}>
              ...
              <div>{dictionary.notificationCenter.noNotification}</div>
            )}
          </ul>
        </Link>
      </div>
